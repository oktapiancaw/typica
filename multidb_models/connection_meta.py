...
class ConnectionUriMeta(ConnectionMeta):
    ...
    type_connection: Optional[ConnectionTypes] = Field(None, examples=ConnectionTypes.list())
    clusters: Optional[list[HostMeta]] = Field(None)

    @model_validator(mode='after')
    def extract_uri(self):
        if self.uri:
            uri = re.sub(r'\w+:(//|/)', '', self.uri)
            metadata, others = re.split(r'\/\?|\/', uri) if re.search(r'\/\?|\/', uri) else [uri, None]
            ...
            if '@' in metadata:
                if ',' in metadata:
                    ...
                    self.username, self.password, self.host, self.port = re.split('\@|\:', metadata)
            else:
